...
;(function ($w) {
    ...
    jm.prototype = {
        ...
        add_node: function (parent_node, nodeid, topic, data, stop_invoke_event /** by cy*/) {
            if (this.get_editable()) {
                ...
                if (!!node) {
                    ...
                    if (!stop_invoke_event) {
                        /** by cy*/
                        this.invoke_event_handle(jm.event_type.edit, {
                            evt: 'add_node',
                            data: [node.parent.id, nodeid, topic, data],
                            node: nodeid
                        });
                    }
                }
        ...
        insert_node_before: function (node_before, nodeid, topic, data, stop_invoke_event /** by cy*/) {
            if (this.get_editable()) {
        ...
        insert_node_after: function (node_after, nodeid, topic, data, stop_invoke_event/** by cy*/) {
            if (this.get_editable()) {
        ...
        remove_node: function (node, stop_invoke_event/** by cy*/) {
            if (!jm.util.is_node(node)) {
                var the_node = this.get_node(node);
                if (!the_node) {
                    logger.error('the node[id=' + node + '] can not be found.');
        ...
        update_node: function (nodeid, topic, stop_invoke_event/** by cy*/) {
            if (this.get_editable()) {
        ...
        // 计算内容的行数，并设置容器的高度
        auto_height_node_before_update: function (node) {
            const topic = node.topic
            if (!!topic) {
                const lines = topic.includes('<br/>') ? topic.split('<br/>').length : topic.split('\n').length
        ...
        move_node: function (nodeid, beforeid, parentid, direction, stop_invoke_event/** by cy*/) {
            if (this.get_editable()) {
    ...
    jm.view_provider.prototype = {
        init: function () {
            ...
            this.e_editor = $c('textarea'); // by cy
            ...
            this.e_editor.style.fontSize = '11px'; // by cy
        ...
        create_node_element: function (node, parent_node) {
            ...
            this._reset_node_custom_style(node, d, node.data);
        ...
        edit_node_begin: function (node) {
            ...
            this.e_editor.style.height = (element.clientHeight - parseInt(ncs.getPropertyValue('padding-top')) - parseInt(ncs.getPropertyValue('padding-bottom'))) + 'px'; // by cy
            element.innerHTML = '';
        ...
        reset_node_custom_style: function (node) {
            this._reset_node_custom_style(node, node._data.view.element, node.data);
        },

        _reset_node_custom_style: function (node, node_element, node_data) {
            ...
            if (node.isroot) {
                node_element.style.backgroundColor = '#289f45'
            } else {
                node_element.style.fontSize = '12px';
            }
