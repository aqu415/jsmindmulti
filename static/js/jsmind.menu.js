  ...
  Jm.menu.prototype = {
      ...
      d.addEventListener('click', function () {
        if (this.selected_node || !isDepNode) {
          ...
          if (!_this._get_mid_opts()) {
            ...
            fn.call(j, Jm.util.uuid.newid(), this.menuOpts.newNodeText || '请输入名称')
            return
          }
          cb(this.selected_node, _this._mid_stage_next(function () {
            ...
            argus[1] = this.menuOpts.newNodeText || '请输入名称'
            if (argus[0]) {
              retArgs = argus
            }
            fn.apply(j, retArgs)
    ...
    _menu_default_mind_methods: {
      ...
      _menu_edit_node_begin (scope, node) {
        ...
        scope.e_editor.style.height = (element.clientHeight - parseInt(ncs.getPropertyValue('padding-top')) - parseInt(ncs.getPropertyValue('padding-bottom'))) + 'px'
        element.innerHTML = ''
